...
function parseMeta(text) {
  ...
  return {
    ...
    openingBalance: parseAmount(
      grab(/Opening Balance.*?Ledger\s*:?\s*([0-9,]+\.\d{2})/is, text) ||
      grab(/Opening Balance\D*([0-9,]+\.\d{2})/i, text)),
  };
}

function parseTotals(text) {
  const count = (re) => { const v = grab(re, text).replace(/,/g, ""); return /^\d+$/.test(v) ? parseInt(v, 10) : null; };
  return {
    totalDrCount: count(/Total DR Transactions\s*:?\s*([0-9,]+)/i),
    totalCrCount: count(/Total CR Transactions\s*:?\s*([0-9,]+)/i),
    sumDr: parseAmount(grab(/Sum of DR Transactions\s*:?\s*([0-9,]+\.\d{2})/i, text)),
    sumCr: parseAmount(grab(/Sum of CR Transactions\s*:?\s*([0-9,]+\.\d{2})/i, text)),
    availableBalance: parseAmount(grab(/Available Balance\s*:?\s*([0-9,]+\.\d{2})/i, text)),
    closingBalance: parseAmount(grab(/Closing Ledger Balance\s*:?\s*([0-9,]+\.\d{2})/i, text)),
  };
}
...
async function convertPdf(arrayBuffer, options = {}) {
  ...
  const statement = {
    meta: parseMeta(fullText),
    totals: parseTotals(fullText),
